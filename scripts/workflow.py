...
class Workflow:
    ...
    def create(cls, path, runtime, scope, affected_modules, operation='upgrade', change_type='content',
               mode='standard', budget=None, authorization=None):
        ...
        with self.transaction() as db:
            ...
            config = dict(version=1, runtime=str(Path(runtime).resolve()), scope=scope,
                          ...
                          initial_files=runtime_files(runtime))
    ...
    def authorize_more(self, calls, authorization):
        ...
        with self.transaction() as db:
            ...
            db.execute('INSERT INTO events(kind,value) VALUES (?,?)', ('authorization', encoded({'additional_calls': calls, 'authorization': authorization})))

    def reserve(self, item, role, request, candidate=None, retry=False):
        if role not in ('candidate', 'baseline', 'neighbor', 'grader', 'delegated') or not item:
        ...
        with self.transaction() as db:
            ...
            if cfg['stopped'] or used >= cfg['budget']:
            ...
            if cfg['mode'] == 'standard':
                ...
                if role == 'candidate':
                    ...
                    cap = 4 if cfg['operation'] == 'new' else 2
                ...
                if role == 'grader':
                    ...
                    if item not in current and current:
                        raise DispatchStopped('standard permits one review per pass')
...
def main():
    ...
    for command in ('status', 'repair', 'stop', 'checkpoint', 'authorize-more', 'reserve', 'finish'):
        ...
        if command == 'authorize-more':
            p.add_argument('--calls', type=int, required=True); p.add_argument('--authorization', required=True)
        if command == 'reserve':
    ...
    try:
        if args.command == 'init':
            ...
            wf = Workflow.create(args.workflow, args.runtime, plan['scope'], plan['affected_modules'],
                                 plan.get('operation', 'upgrade'), plan.get('change_type', 'content'), args.mode, args.budget, args.authorization)
            wf.note('initial_inspection', plan)
        else:
            wf = Workflow(args.workflow)
            if args.command == 'repair': wf.repair()
            elif args.command == 'stop': wf.stop()
            elif args.command == 'authorize-more': wf.authorize_more(args.calls, args.authorization)
